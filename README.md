...
#### Pointing ScidUp at libscid-cpp and Tcl/Tk

`dev` / `dev-no-checks` do not hard-code dependency prefixes.

libscid-cpp must be discoverable by CMake as a package. Point `CMAKE_PREFIX_PATH` at
the libscid-cpp install prefix, or install libscid-cpp into a standard CMake search path.
...
cmake --preset dev -DCMAKE_PREFIX_PATH="/path/to/libscid-cpp/prefix;$DEPS_INSTALL_PREFIX"
```

Otherwise, if your Tcl/Tk is not in a default search path, pass a prefix explicitly:

```sh
cmake --preset dev -DCMAKE_PREFIX_PATH="/path/to/libscid-cpp/prefix;/path/to/tcltk/prefix;$CMAKE_PREFIX_PATH"
```
