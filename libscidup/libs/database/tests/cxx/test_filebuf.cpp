...
static const char* filename = SCIDUP_TEST_RESOURCES_DIR "res_utf8.txt";
...
lines getlineScid(scid::database::Filebuf& file, size_t bufSize) {
	lines vScid;
	std::unique_ptr<char[]> unique_buf(new char[bufSize]);
	char* buf = unique_buf.get();
	*buf = 0;
...
TEST_P(Test_FilebufGetline, filesize) {
	...
	{ // Verify that FilebufAppend::size() is correct.
		scid::database::FilebufAppend fileAppend;
		fileAppend.open(filename, scid::database::FMODE_ReadOnly);
