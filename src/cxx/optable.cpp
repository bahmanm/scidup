...
{
    GameNumber = 0;
    White = scid::database::strDuplicate("");
    Black = scid::database::strDuplicate("");
    Site = scid::database::strDuplicate("");
    WhiteID = BlackID = 0;
    WhiteElo = BlackElo = 0;
    AvgElo = 0;
              ...
              scid::core::uint maxExtraMoves, scid::core::uint maxThemeMoveNumber)
{
    ...
    while (i < columnMoves) {
        ...
        if (!sm) {
            ...
            ASSERT(position);
            position->MakeSANString (&*sm, Move[i], scid::core::SAN_CHECKTEST);
...
{
    if (! WTM) {
        scid::database::strCopy (line->Move[0], StartLine[0]);
    ...
    if (WTM  && scid::database::strEqual (line->Move[0], ExcludeMove)) { return false; }
    if (!WTM  &&  scid::database::strEqual (line->Move[1], ExcludeMove)) { return false; }
...
OpTable::SortTableLines (OpLine ** lines, scid::core::uint nlines, scid::core::uint depth)
{
    ...
    for (i=0; i < nlines; i++) {
        bool newMove = true;
        for (j=0; j < nUnique; j++) {
            if (scid::database::strEqual (lines[i]->GetMove(depth), moves[j].move)) {
                newMove = false;
                moves[j].count++;
    ...
    for (i=0; i < nUnique-1; i++) {
        for (j=i+1; j < nUnique; j++) {
            ...
            if (result == 0) {
                result = scid::database::strCompare (moves[i].move, moves[j].move);
            }
            // Ensure short lines (with no more moves) come first:
            if (scid::database::strEqual (moves[i].move, "")) { result = -1; }
            if (scid::database::strEqual (moves[j].move, "")) { result = 1; }
            // Swap if necessary:
            if (result > 0) {
...
OpTable::PrintLaTeX (scid::core::DString * dstr, const char * title, const char * comment)
{
    ...
    dstr->Append ("\\hline\n");
    if (! scid::database::strEqual (comment, "")) {
...
OpTable::PrintHTML (scid::core::DString * dstr, const char * title, const char * comment)
{
    ...
    dstr->Append ("</table>");
    if (! scid::database::strEqual (comment, "")) { dstr->Append (comment, "\n"); }
    dstr->Append ("</center>");
...
OpTable::TopEcoCodes (scid::core::DString * dstr, scid::core::uint count)
{
    ...
    for (scid::core::uint n=1; n <= count; n++) {
        ...
        if (maxFreq > 0) {
            char ecoStr[8];
            scid::database::strCopy (ecoStr, "A00-E99");
                       ...
                       scid::core::MovetextLocation location)
{
    ...
    while (1) {
        ...
        if (MoveOrder[index].count == MoveOrder[index-1].count  &&
            scid::database::strCompare(MoveOrder[index].moves, MoveOrder[index-1].moves) > 0) {
            break;
        }
